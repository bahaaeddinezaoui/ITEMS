...
const ConsumablesBrandsPage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setError(t('consumableBrands.saveError', 'Failed to save brand') + ': ' + (err.response?.data?.error || err.message));
        } finally {
            setSaving(false);
        }
    };
            ...
            <div className="card" style={{ background: 'var(--color-bg-card)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-lg)', overflow: 'hidden' }}>
                ...
                <div style={{ padding: 'var(--space-4)', borderBottom: '1px solid var(--color-border)', display: 'flex', gap: 'var(--space-3)', alignItems: 'center' }}>
                    <div style={{ position: 'relative', flex: '0 1 320px', minWidth: '180px' }}>
                        ...
                        <input type="text" placeholder={t('consumableBrands.searchPlaceholder', 'Search brands...')} className="form-input" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={{ paddingLeft: 'var(--space-10)', height: '40px', background: 'var(--color-bg-card)' }} />
                    </div>
                ...
                <div style={{ overflowY: 'auto', maxHeight: 'calc(100vh - 340px)' }}>
                    {loading ? (
                        <div className="loading-state" style={{ padding: 'var(--space-12)' }}>
                            ...
                            <span>{t('consumableBrands.loading', 'Loading...')}</span>
                        </div>
