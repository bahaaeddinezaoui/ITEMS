...
const AssetsBrandsPage = () => {
    ...
    const fetchBrands = async () => {
        ...
        try {
            ...
            setError(t('assetBrands.fetchError', 'Failed to fetch brands') + ': ' + err.message);
            setBrands([]);
        } finally {
            setLoading(false);
        }
    ...
    const handleSubmit = async (e) => {
        ...
        if (!brandName) {
            setError(t('assetBrands.enterBrandName', 'Please enter a brand name'));
            return;
        }
        ...
        try {
            if (editingBrand) {
                ...
                if (brandForm.brand_photo && typeof brandForm.brand_photo !== 'string') {
                    ...
                    await assetBrandService.update(editingBrand.asset_brand_id, formData);
                } else {
                    if (hasTranslations) payload.translations = formTranslations;
                    await assetBrandService.update(editingBrand.asset_brand_id, payload);
                }
            } else {
                const formData = new FormData();
    ...
    const handleDelete = async (id) => {
        if (!window.confirm(t('assetBrands.confirmDelete', 'Delete this brand?'))) return;
        try {
            await assetBrandService.delete(id);
            await fetchBrands();
        } catch (err) {
            setError(t('assetBrands.deleteError', 'Failed to delete brand') + ': ' + err.message);
        }
    };
                ...
                <div style={{ overflowY: 'auto', maxHeight: 'calc(100vh - 340px)' }}>
                    {loading ? (
                        <div className="loading-state" style={{ padding: 'var(--space-12)' }}>
                            ...
                            <span>{t('assetBrands.loading', 'Loading...')}</span>
                        </div>
                    ) : filteredBrands.length === 0 ? (
                        <div className="empty-state" style={{ padding: 'var(--space-12)' }}>
                            <Tag size={48} style={{ marginBottom: 'var(--space-4)', opacity: 0.3 }} />
                            <p style={{ color: 'var(--color-text-muted)' }}>
                                {searchTerm || statusFilter ? t('assetBrands.noMatchingBrands', 'No matching brands') : t('assetBrands.noBrands', 'No brands yet')}
                            </p>
                        </div>
                    ) : (
                        filteredBrands.map(brand => (
                            <div
                                key={brand.asset_brand_id}
                                style={{
                                    padding: 'var(--space-4) var(--space-5)',
                                    display: 'flex',
                            ...
                            >
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', flexWrap: 'wrap' }}>
                                            ...
                                            {brand.is_active === false && (
                                                <span style={{ fontSize: 'var(--font-size-xs)', background: 'rgba(239, 68, 68, 0.1)', color: 'var(--color-error)', padding: '1px 6px', borderRadius: 'var(--radius-sm)' }}>
                                                    {t('assetBrands.inactive', 'Inactive')}
                                                </span>
                                            )}
                                        </div>
                                    </div>
                                ...
                                {authService.isSuperuser() && (
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-1)', flexShrink: 0 }}>
                                        ...
                                        <button onClick={() => handleDelete(brand.asset_brand_id)} className="btn btn-secondary" style={{ padding: 'var(--space-1)', width: '28px', height: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--color-error)' }} title={t('common.delete')}>
                                            <Trash2 size={14} />
                                        </button>
                                    </div>
                                )}
...
export default AssetsBrandsPage;
