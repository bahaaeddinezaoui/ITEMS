...
const StockItemInstanceCreatePage = () => {
    ...
    useEffect(() => {
        const load = async () => {
            ...
            try {
                ...
                setModel(m || null);
                // Fetch default consumables for this stock item model
                try {
                    const defData = await stockItemModelService.getDefaultConsumables(modelIdParam);
                    const items = Array.isArray(defData) ? defData : (defData?.results || []);
                    setDefaultConsumables(items);
                } catch { setDefaultConsumables([]); }
            } catch (e) {
                setModel(null);
    ...
    return (
        <div className="page-container" style={{ maxWidth: '1200px', margin: '0 auto' }}>
            ...
            {/* Default Consumables Awareness */}
            {!loading && model && defaultConsumables.length > 0 && (
                <div style={{
                    display: 'flex', alignItems: 'center', gap: 'var(--space-3)',
                    ...
                    <Droplets size={18} style={{ color: '#f59e0b', flexShrink: 0 }} />
                    <span style={{ color: 'var(--color-text-secondary)' }}>
                        {t('stockItemInstanceCreate.defaultConsumablesWillBeCreated', {
                            count: defaultConsumables.length,
                            names: defaultConsumables.map(dc => `${dc.consumable_model_name || '?'} (×${dc.quantity || 1})`).join(', ')
                        })}
                    </span>
                </div>
            )}
