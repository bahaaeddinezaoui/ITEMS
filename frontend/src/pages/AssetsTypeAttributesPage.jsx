...
const AssetsTypeAttributesPage = () => {
    ...
    useEffect(() => {
        if (!typeId) {
            setError(t('assetsTypeAttributes.typeIdRequired'));
            return;
        }
        fetchAll();
    }, [typeId]);
    ...
    const fetchAll = async () => {
        ...
        try {
            const [type, defs, attrs] = await Promise.all([
                assetTypeService.getById(typeId),
                assetAttributeDefinitionService.getAll(),
                assetTypeAttributeService.getByAssetType(typeId),
            ]);
            setAssetType(type);
            setAttributeDefinitions(Array.isArray(defs) ? defs : []);
            setTypeAttributes(Array.isArray(attrs) ? attrs : []);
        } catch (err) {
            setError(t('assetsTypeAttributes.loadError') + ': ' + err.message);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!form.asset_attribute_definition) {
            setError(t('assetsTypeAttributes.selectDefinition'));
            return;
        }
        setSaving(true);
        setError(null);
        try {
            const payload = {
                ...
                asset_attribute_definition: Number(form.asset_attribute_definition),
                is_mandatory: form.is_mandatory,
                default_value: form.default_value || null,
            };
            await assetTypeAttributeService.create(payload);
            setForm({ asset_attribute_definition: '', is_mandatory: false, default_value: '' });
            setShowForm(false);
            await fetchAll();
        } catch (err) {
            setError(t('assetsTypeAttributes.addError') + ': ' + err.message);
        } finally {
            setSaving(false);
        }
    };
    ...
    const handleDelete = async (definitionId) => {
        if (!window.confirm(t('assetsTypeAttributes.confirmRemove'))) return;
        try {
            await assetTypeAttributeService.delete(Number(typeId), Number(definitionId));
            await fetchAll();
        } catch (err) {
            setError(t('assetsTypeAttributes.removeError') + ': ' + err.message);
        }
    };

    const usedDefinitionIds = useMemo(() => {
        return new Set(typeAttributes.map(a => a.asset_attribute_definition));
    }, [typeAttributes]);

    const availableDefinitions = useMemo(() => {
        return attributeDefinitions.filter(d => !usedDefinitionIds.has(d.asset_attribute_definition_id));
        ...
        <div style={{ height: 'calc(100vh - 100px)', display: 'flex', flexDirection: 'column' }}>
            ...
            <div className="card" style={{ overflow: 'hidden', display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
                <div className="card-header" style={{
                    ...
                    <h2 style={{ fontSize: 'var(--font-size-md)', fontWeight: '600', margin: 0 }}>{t('assetsTypeAttributes.attributes')}</h2>
                    <button
                    ...
                    <div style={{ padding: 'var(--space-4)', borderBottom: '1px solid var(--color-border)', backgroundColor: 'var(--color-bg-tertiary)' }}>
                        <form onSubmit={handleSubmit}>
                            <select
                                ...
                                value={form.asset_attribute_definition}
                                onChange={handleChange}
                                style={{ width: '100%', marginBottom: 'var(--space-2)', padding: 'var(--space-2)' }}
                            >
                                <option value="">{t('assetsTypeAttributes.selectDefinition')}</option>
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-2)', marginBottom: 'var(--space-2)' }}>
                                ...
                                <input
                                    type="text"
                                    name="default_value"
                                    placeholder={t('assetsTypeAttributes.defaultValuePlaceholder')}
...
export default AssetsTypeAttributesPage;
