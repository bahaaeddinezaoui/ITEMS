...
const ConsumableModelCompatibilityPage = () => {
    ...
    const fetchAll = useCallback(async () => {
        ...
        try {
            ...
            setError(t('consumableModelCompatibility.fetchError') + ': ' + err.message);
        } finally {
            setLoading(false);
        }
    }, [modelId]);
    ...
    const addCompatibility = async () => {
        ...
        try {
            ...
            setError(t('consumableModelCompatibility.addError') + ': ' + err.message);
        }
    };
    ...
    const removeCompatibility = async (assetModelId) => {
        ...
        try {
            ...
            setError(t('consumableModelCompatibility.removeError') + ': ' + err.message);
        }
    };
        ...
        <div style={{ height: 'calc(100vh - 100px)', display: 'flex', flexDirection: 'column' }}>
            ...
            <div className="card" style={{ overflow: 'hidden', display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
                <div className="card-header" style={{
                    ...
                    <h2 style={{ fontSize: 'var(--font-size-md)', fontWeight: '600', margin: 0 }}>{t('consumableModelCompatibility.compatibleAssetModels')}</h2>
                    {isSuperuser && (
                        <button
                        ...
                        >
                            {t('consumableModelCompatibility.addCompatibleModel')}
                        </button>
                    )}
                </div>
                ...
                <div style={{ overflowY: 'auto', flex: 1 }}>
                    ...
                    {compatibleAssetModels.length === 0 && !loading && (
                        <div style={{ padding: 'var(--space-4)', color: 'var(--color-text-secondary)', textAlign: 'center' }}>
                            {t('consumableModelCompatibility.noCompatibleAssetModels')}
                        </div>
                    )}
                </div>
            </div>
                    ...
                    <div className="modal-overlay" onClick={() => { setShowAddForm(false); setSelectedAssetModelId(''); }}>
                        <div className="modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '640px', width: '90vw', minHeight: '500px' }}>
                            <div className="modal-header">
                                <h3 className="modal-title">{t('consumableModelCompatibility.addCompatibleModel')}</h3>
                            ...
                            <div className="modal-body">
                                <div className="form-group" style={{ marginBottom: 'var(--space-4)' }}>
                                    <label className="form-label" style={{ fontWeight: '600', marginBottom: 'var(--space-2)', display: 'block' }}>
                                        {t('consumableModelCompatibility.selectAssetModel')}
                                    </label>
                                    <SearchableSelect
                                        value={selectedAssetModelId}
                                        onChange={(e) => setSelectedAssetModelId(e.target.value)}
                                        placeholder={t('consumableModelCompatibility.selectAssetModel')}
