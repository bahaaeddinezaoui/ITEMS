...
export default function Stepper({
  ...
  const direction = controlledDirection !== undefined ? controlledDirection : internalDirection;
  ...
  const updateStep = (newStep, dir) => {
    if (!isControlled) {
      ...
      if (dir !== undefined) setInternalDirection(dir);
    }
  ...
  const handleBack = () => {
    if (currentStep > 1) {
      if (typeof onBeforeStepChange === 'function') {
        const allowed = onBeforeStepChange(currentStep - 1, currentStep);
  ...
  const handleComplete = () => {
    if (typeof onBeforeStepChange === 'function') {
      const allowed = onBeforeStepChange(totalSteps + 1, currentStep);
      if (allowed === false) return;
    }
  ...
  return (
    <div className="stepper-outer-container" {...rest}>
      <div className={`stepper-step-circle-container ${stepCircleContainerClassName}`}>
        <div className={`stepper-step-indicator-row ${stepContainerClassName}`}>
              ...
              <React.Fragment key={stepNumber}>
                {renderStepIndicator ? (
                  renderStepIndicator({
                    ...
                    onStepClick: clicked => {
                      if (typeof onBeforeStepChange === 'function') {
                        const allowed = onBeforeStepChange(clicked, currentStep);
                        if (allowed === false) return;
                      }
                      const dir = clicked > currentStep ? 1 : -1;
                      updateStep(clicked, dir);
                    }
                  })
                ) : (
                  <StepIndicator
        ...
        <StepContentWrapper
          ...
          currentStep={currentStep}
          direction={direction}
          className={`stepper-step-content-default ${contentClassName}`}
        >
...
function StepIndicator({ step, currentStep, onClickStep, disableStepIndicators }) {
  ...
  return (
    <motion.div onClick={handleClick} className="stepper-step-indicator" style={disableStepIndicators ? { pointerEvents: 'none', opacity: 0.5 } : {}} animate={status} initial={false}>
      <motion.div
        variants={{
          inactive: { scale: 1, backgroundColor: 'var(--color-bg-secondary)', color: 'var(--color-text-secondary)' },
          active: { scale: 1, backgroundColor: 'var(--color-primary)', color: 'var(--color-primary)' },
          complete: { scale: 1, backgroundColor: 'var(--color-primary)', color: 'var(--color-success)' }
        }}
